...
fn set_password() {
    ...
    keytar::ffi::set_password(service.clone(), account.clone(), password_in.clone()).unwrap();

    let password_out = keytar::ffi::get_password(service.clone(), account.clone()).unwrap();
    assert_eq!(true, password_out.success);
    assert_eq!(password_in, password_out.password);

    keytar::ffi::delete_password(service.clone(), account.clone()).unwrap();
}

#[test]
fn get_not_existent_password() {
    let password_out = keytar::ffi::get_password("doesn't".to_owned(), "exist".to_owned()).unwrap();
...
fn delete_password() {
    ...
    let password_out = keytar::ffi::get_password(service, account).unwrap();
    assert_eq!(false, password_out.success);
}
...
fn find_password() {
    ...
    let password_out = keytar::ffi::find_password(service.clone()).unwrap();
    assert_eq!(true, password_out.success);
    assert_eq!(password_in, password_out.password);
